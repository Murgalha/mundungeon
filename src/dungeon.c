...
// ======================
bool tile_is_corner(Dungeon *dungeon, V2 tile) {
...
V2 get_random_wall(Dungeon *dungeon) {
	V2 wall;
...
V2 get_random_corridor(Dungeon *dungeon) {
	V2 corridor;
...
void dungeon_fill_rect(Dungeon *dungeon, V2 begin, int width, int height, char element) {
	V2 end;
	end.x = begin.x + width;
...
// =========================
void dungeon_make_room_at(Dungeon *dungeon, V2 begin, int width, int height) {
	V2 end, tile;
	end.x = begin.x + width;
	end.y = begin.y + height;
	...
	dungeon_fill_rect(dungeon, begin, width, height, FLOOR);
}

bool dungeon_has_room_space(Dungeon *dungeon, V2 door, int width, int height, Direction d) {
...
bool dungeon_make_random_room(Dungeon *dungeon) {
	V2 corridor;
	int counter = 0;
	do {
	...
	for(Direction d = 0; d < 4; d++) {
		if(dungeon_has_room_space(dungeon, corridor, size, size, d)) {
			V2 begin, door;
			int width, height;
			width = height = size;
			...
			dungeon_make_room_at(dungeon, begin, size, size);
...
// =========================
void dungeon_make_corridor_at(Dungeon *dungeon, V2 door, int size, Direction d) {
...
bool dungeon_has_corridor_space(Dungeon *dungeon, V2 begin, int size, Direction d) {
	V2 vector = direction_vector(d);
	V2 tmp;
...
void dungeon_make_random_corridor(Dungeon *dungeon) {
	V2 wall;
	int counter = 0;
	do {
	...
	// algorithm stays "more random"
	for(Direction d = 0; d < 4; d++) {
		if(dungeon_has_corridor_space(dungeon, wall, size, d)) {
			dungeon_make_corridor_at(dungeon, wall, size, d);
			return;
		}
	}

	printf("Could not create random corridor\n");
	return;
}
// =========================
...
Dungeon *dungeon_new(u16 size) {
	...
	dungeon_fill_rect(dungeon, d_begin, size, size, EMPTY);

	// Creating initial room in the middle of the map
	d_begin.x = size/2 - 5;
	d_begin.y = size/2 - 3;
	dungeon_make_room_at(dungeon, d_begin, 10, 7);

	int k = 0;
	while(k != 5) {
		dungeon_make_random_corridor(dungeon);
		if(!dungeon_make_random_room(dungeon)) {
			k++;
		}
		else {
			k = 0;
